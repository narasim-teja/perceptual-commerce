...
The spine — intent → policy gate → fail-closed authorize → mint scoped card → settle →
idempotency — is fixed. Perception sources and settlement rails are pluggable.
...
and reorders — no human in the loop — but only if the policy contract allows the mint, and the
...
| `frontend` | Next.js console — the demo control surface, and where the vision layer runs |
